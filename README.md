...
(also note that the 'result' above is a dictionary that includes the following keys: 'options': copy of the kwargs input options, 'segdata': segment data for selected segment, 'ps_segments': dictionary of contents of ps_segments.dat, and 'df_tide': Pandas dataframe containing output of the tide predictions):
