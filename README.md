...
Modified to run in Python 3.x by Greg Pelletier (tested using Python 3.12 & 3.13)
...
Next we show how to predict the time series of tides in all of the segments, and interpolate the predicted tide series to a grid of the entire Puget Sound. The grid we are using is the subset of the [LiveOcean](https://faculty.washington.edu/pmacc/LO/LiveOcean.html) ROMS grid that contains the pstide segments. Using grid=True activates pstide to perform the gridded prediction of tides. This example produces a netcdf file (tides_gridded.nc), which contains the tides for the selected options of one day (length=1.0), on the selected date (start='2024-08-01'), in every grid cell at each 5-minute time step (interval=5). An xarray dataset of the gridded predictions is added to the result dictionary (result['tides_gridded']). This example also produces the animated gif (tides_gridded.gif) of the predictions that is shown below with the selected frame rate of 10 frames per second (fps=10).   
```
from pstide import run_pstide
...
map_segments()
```
<img width="2455" height="3429" alt="pstide_segments" src="https://github.com/user-attachments/assets/79c52b24-ee49-4997-9c37-573c8d6a551f" />
...
run_pstide(**kwargs)
    ...
    Returns.
        dictionary of all results including the following:
            ...
            tides_all_segments: dataframe of tide predictions for the all segments (meters MLLW)
            tides_selected_segment: dataframe of tide predictions for the selected segment
